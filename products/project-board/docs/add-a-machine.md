...
| 9. Start the agents (one agent by hand: [enroll an agent](repo:app-ecosystem/products/project-board/packages/project-board/src/project_board/procedures/enroll-an-agent.md)) | Your agent | One start script per agent, holding its folder and every flag, and one `tmux` session per agent, named after it. | Routine. |
