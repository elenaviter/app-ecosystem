...
| 8. Workspaces | Either | One empty folder per agent, under the host's agent workspace root (`<root>/<agent alias>`; set with `pb host configure --agent-workspace-root <path>`, else the first approved work root). An agent started anywhere else still gets that folder from `pb worker context`, never the folder its session started in. Each agent clones the project's repositories into it when it joins (step 12). | Routine. |
