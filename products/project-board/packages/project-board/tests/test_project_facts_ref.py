"""`pb worker context` names the project's standing pages (W262, W317).
...
context names it so every agent reads it at start. The development environment
page sits beside it and gives a new host the project's reproducible setup. A
project without either page gets empty values, never a ref to nothing.
"""
...
from project_board.client.journals import (
...
def test_the_context_names_the_project_environment_page_when_it_exists(tmp_path):
    workspace, home = _workspace(tmp_path)
    (home / PROJECT_ENVIRONMENT_FILE).write_text(
        "# Project one: development environment\n", encoding="utf-8"
    )

    context = workspace.context(PROJECT)

    assert context["project_environment_ref"] == (
        "repo:journals/projects/project-one/project-environment.md"
    )
    assert context["local_project_environment"] == str(
        home / PROJECT_ENVIRONMENT_FILE
    )
...
def test_a_project_without_the_page_gets_no_ref(tmp_path):
    ...
    assert context["project_environment_ref"] == ""
    assert context["local_project_environment"] == ""
