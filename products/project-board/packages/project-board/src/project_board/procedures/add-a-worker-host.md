...
| runtime of each agent | `claude-code` for both, or one of each | an agent is its runtime plus its session id, and that identity never changes. One host can run Claude Code and Codex agents side by side, each in its own workspace. Needing another runtime means adding another agent. |
