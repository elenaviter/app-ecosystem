...
# One spelling of a path: its directory with symlinks resolved, its name kept.
# Not `realpath -m`, which macOS's realpath refuses (W346).
canon() { local dir; dir=$(cd -P -- "$(dirname -- "$1")" 2>/dev/null && pwd) &&
