...
| A change that adds, removes or renames a board operation (an operation id in `project_board/contract/worker_operation_contract.py`) | the platform rebuild that stages the matching `project-board` package (the `app-ecosystem` row above) **first**, then the app deploy. The board checks its handlers against the operation contract inside the platform image and refuses to load while they differ. Find the case from the diff with the check below, before the window is planned | a board reload alone: the board fails to load with `Problem Board operation policy and handler table differ` until the image catches up (2026-09-26, W326: down 03:32–03:36Z) |
...
Run this before planning a window, from the App Ecosystem checkout, with the
commit the runtime runs now and the commit to deploy:
...
Any output names an operation id that is new, removed or renamed. The window
is then a platform rebuild with the `project-board` package staged, **then** the
board deploy, never a board reload alone. No output: the board reload may go on
its own.
...
build, `pb procedure verify` for the package. For an app, the reload's receipt line
`Loaded: mounted tree at head <sha>, clean` names what loaded and `git -C <deploy-worktree> rev-parse
HEAD` the commit on disk: compare both with the approved candidate before anything else. A
`Loaded: snapshot of ...` line means an `activation` block is still in effect. A bundle reload reports how many modules it evicted;
