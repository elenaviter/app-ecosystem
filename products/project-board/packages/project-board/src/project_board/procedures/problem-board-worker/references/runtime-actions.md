...
build, `pb procedure verify` for the package. For an app, the chat-proc log names the path and
commit it loaded, and `git -C <deploy-worktree> rev-parse HEAD` names the commit on disk: compare
both with the approved candidate before anything else. A bundle reload reports how many modules it evicted;
