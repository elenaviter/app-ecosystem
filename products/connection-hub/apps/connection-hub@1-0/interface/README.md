...
| `project_person_my_card_seed` | POST | operations | Migrate an existing person's prior positive selection into the untouched My Card created by `project_person_control_create`. The authenticated actor must be a project administrator, and the Control Card must carry the migration provenance marker; otherwise the operation returns `project_person_my_card_seed_not_migrated`. Connection Hub reconciles the submitted selection with the active catalog, intersects every resource, operation, named-service, and account dimension with the current Control Card, and writes no unrelated Card field. Revision 1 plus a durable seed marker makes the operation one-shot and exactly replayable. New people keep the empty My Card created by the normal lifecycle. |
