...
|`authority_provider_set`|POST|operations|Administrator, CSRF: apply the buffer to the staged `bundles.yaml` through the platform's descriptor editor: one provider block replaced, comments and every other key kept, a `.bak-<stamp>` beside the file, secret-bearing keys merged from the file; refuses a dropped secret key unless `allow_secret_removal`. Answers the refreshed authorities, the edit (path, backup, changed) and `activation` (`refresh` until the live reload lands).|
|`platform_sign_in_set`|POST|operations|Administrator, CSRF: point the platform at another sign-in provider of its authority by changing `auth.type` and `auth.connection_hub.provider_id` together in the staged `assembly.yaml` (backup beside it). The lane applies on a runtime refresh, by design.|
...
| `delegated_access_update` | POST | operations | Replace or preserve the selected dimensions of an existing owner-scoped card in place. The credential remains bound to the same `access_id`. Optional `accepted_operations` (`{resource: [operation]}`) accepts a changed descriptor for exactly those selected operations; every other changed selected operation stays suspended. |
