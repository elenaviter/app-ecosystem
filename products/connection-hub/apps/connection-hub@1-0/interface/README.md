...
|`delegated_access_renew`|POST|operations|Issue a fresh token on an existing manual card, expired or live, keeping every grant, selection, account binding and policy; the previous token is retired; the new token returns once, as at creation. Optional `ttl_seconds` (default: the card's previous lifetime). Refuses revoked cards and non-manual families (`delegated_access_renew_unsupported`).|
