...
| `delegated_access_list` | GET | operations | List automation credentials and the configured resource/grant catalog. Named-service resources include their descriptor-backed namespace operation tree and provider-declared connected-account requirements. Each item also carries `caller_profile` and `stable_identity` for a resident agent card, `resource_acceptance` (the descriptor revision and digests each resource was accepted at), `resource_offers` (which other resources may join the card and why the rest may not), and `catalog_drift.resources` plus `catalog_drift.changed` for per-resource drift and suspended operations. |
...
| `project_person_control_get` | POST | operations | Read one project-held live-person or pending-invitation Control Card. Live coordinates are `project_ref` + `target_subject`; pending coordinates are `project_ref` + `invitation_ref` + deterministic `control_id`. The authenticated actor is authorized by the host-bound `ProjectAuthorizationPort`; payload identity or role claims confer no authority. |
| `project_person_control_create` | POST | operations | Create a deterministic project-held Control Card with an exact initial catalog selection. A live-person request supplies `target_subject`; the lifecycle records the identity edge and creates an empty My Card. A pending request supplies `invitation_ref` and `target_email`; only the normalized email digest is retained, and no person Card or edge exists before binding. Composition is fixed to `and`. Live-person `migration` and `project_creation` origins admit their one-shot My Card seed. |
...
| `project_person_control_bind_invitation` | POST | operations | Bind a provider-verified pending invitation to the signed-in person. Exact project, invitation, pending Card id, person, and email-digest evidence are required. The transition creates the deterministic live Control Card, creates the person's initial My Card with the full reviewed selection, records the project identity edge, and consumes the pending Card. Exact retries repair partial commits without resetting later user narrowing. |
| `project_person_my_card_seed` | POST | operations | Establish the positive selection in the untouched My Card created by `project_person_control_create` for an existing-person migration or a new-project creator. The authenticated actor must be a project administrator, and the Control Card must carry exactly one immutable `migration` or `project_creation` origin. Connection Hub reconciles the submitted selection with the active catalog, intersects every resource, operation, named-service, and account dimension with the current Control Card, and writes no unrelated Card field. Revision 1 plus a durable seed marker makes the operation one-shot and exactly replayable. Ordinary new people keep the empty My Card created by the lifecycle. |
| `project_operation_authorize` | POST | operations | Evaluate one exact project resource operation for the authenticated session subject against the active catalog, current per-person Control Card, and current My Card. `ok: true` means the evaluation completed; `allowed` is its allow or deny verdict, with named blocking-boundary evidence. Evaluation failures use `ok: false`. Payload identity, Card, revision, and catalog claims confer no authority. |
...
Pre-membership invitation binding uses a second descriptor-owned provider:
...
The provider runs under the current signed-in session and returns current,
canonical evidence for the exact invitation being accepted. Connection Hub
validates that evidence against the pending Card before creating any person
identity. The pending lifecycle, privacy boundary, full initial My Card, and
