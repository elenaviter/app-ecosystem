...
| `delegated_access_update` | POST | operations | Replace or preserve the selected dimensions and bounded `properties` of an existing owner-scoped Card in place. The credential remains bound to the same `access_id`. Optional `accepted_operations` (`{resource: [operation]}`) accepts a changed descriptor for exactly those selected operations; every other changed selected operation stays suspended. Application API edits preserve the explicit selection marker, including when the reviewed selection is empty. |
