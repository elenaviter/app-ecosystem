...
| `project_person_control_create` | POST | operations | Create a deterministic project-held Control Card with an exact initial catalog selection. A live-person request supplies `target_subject`; the lifecycle records the identity edge and creates a My Card with the same selection as the Control Card, which the person may later narrow. A pending request supplies `invitation_ref` and `target_email`; only the normalized email digest is retained, and no person Card or edge exists before binding. Composition is fixed to `and`. Live-person `migration` and `project_creation` origins admit their one-shot My Card seed. |
...
identity. Binding consumes the pending Card before it creates person-side
