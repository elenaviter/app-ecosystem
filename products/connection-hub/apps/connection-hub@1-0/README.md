...
primary_surfaces:
  ...
  - "remote MCP upstream OAuth — connects an owner-selected MCP through MCP discovery, PKCE, a Client ID Metadata Document, dynamic registration, or a provider-console client, with refresh rotation and revocation"
...
- owner-scoped external MCP connectors, browser OAuth through automatic or
  provider-console client registration, or direct upstream credentials,
  descriptor-drift review, delegated proxy dispatch, and separate
