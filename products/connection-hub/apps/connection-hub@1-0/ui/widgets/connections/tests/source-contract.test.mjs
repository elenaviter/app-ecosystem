...
test('access-card tools are visible and selection doors group their services', () => {
  ...
  assert.match(panel, /open=\{editOpenResources\[resource\] \?\? \(isNew \|\| resourceFamilyRoot\)\}/)
  assert.match(panel, /const resourceFamilyRoots = new Set\(item\.resource_family_roots \|\| \[\]\)/)
  assert.match(panel, /\.\.\.resourceFamilyRoots/)
  assert.match(panel, /resourceFamilyRoot \|\| selectedSelectionChild/)
  assert.match(panel, /const isSelectionRoute = resourceFamilyRoot \|\| Boolean\(/)
  assert.match(panel, /Object\.keys\(item\.resource_grants \|\| \{\}\)\.map\(\(resource\) => \[resource, false\]\)/)
  assert.match(panel, /materializeSelectionRouteGrants\(/)
  assert.doesNotMatch(panel, /Available through/)
  ...
  assert.match(css, /\.resource-selection-family \{[\s\S]*border-left: 2px solid var\(--blue\)/)
  assert.match(css, /\.resource-edit-section \{[\s\S]*width: min\(100%, 980px\)/)
  assert.match(css, /\.resource-edit-section--new \{[\s\S]*width: min\(100%, 940px\)/)
...
test('an exact Control Card reuses the Card editor without joining the agent-card list', () => {
  ...
  assert.match(slice, /projectPersonControl\s*\?\s*'project_person_control_update'\s*:\s*projectAgentCard\s*\?\s*'project_agent_card_update'\s*:\s*projectControlCard\s*\?\s*'project_control_card_update'\s*:\s*'delegated_access_update'/)
...
test('linked Card editing keeps both authority views and dialogs use one governed body layer', () => {
  ...
  assert.doesNotMatch(panel, /effective-composition-guide__legend/)
  assert.match(catalog, /const removedCount = compositionStates\.filter\(\(state\) => state === 'removed'\)\.length/)
  assert.match(catalog, /\{removedCount\} removed by \{composition\?\.controlLabel\}/)
  assert.match(catalog, /\{addedCount\} added by \{composition\?\.controlLabel\}/)
