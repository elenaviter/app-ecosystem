...
const cards = [
  ...
  { access_id: 'e', source: 'oauth', credential_reach: 'multi_resource', expires_at: now + 90 * 86400, resource_grants: { 'x/mcp/worker_stream': ['work:relay'] } },
...
test('by kind separates delivery from reach and labels the holders', () => {
  const groups = groupCards([...cards].reverse(), 'kind', { stateOf, doorLabel })
  assert.deepEqual(groups.map((g) => [g.label, g.records.length]), [
    ['Hosted agents', 1],
    ['Connected clients', 1],
    ['Connected apps', 1],
    ['Issued tokens', 2],
...
test('by state trusts the server flag over the clock', () => {
  ...
  assert.deepEqual(groups.map((g) => [g.label, g.records.map((r) => r.access_id)]), [
    ['Active', ['a', 'e']], ['Expiring soon', ['b']], ['Expired', ['c', 'd']],
  ])
})
