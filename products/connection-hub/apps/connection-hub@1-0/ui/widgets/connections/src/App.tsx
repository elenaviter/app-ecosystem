  ...
  useEffect(() => {
    ...
    const onSurfaceCommand = (event: MessageEvent) => {
      ...
      if (tab === 'delegatedAccess') {
        ...
        try {
          const url = new URL(window.location.href);
          (['pending_agent_grant', 'agent_client_id', 'manual_access_id', 'control_card_id', 'access_id', 'project_ref', 'target_subject', 'oauth_consent', 'resource', 'claims', 'namespace', 'operation', 'outer_operation', 'invocation_policy', 'invocation_change_id', 'request_bound', 'request_digest', 'request_approval_ticket', 'request_card_revision', 'request_authority_revision', 'approval_application_id', 'account_id', 'account_claim'] as const).forEach((key) => {
