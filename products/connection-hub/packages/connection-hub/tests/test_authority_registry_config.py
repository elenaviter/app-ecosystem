...
def test_authority_registry_refuses_retired_bundle_login_types(retired_type):
    registry = _registry()
    registry["authorities"]["kdcube.platform"]["providers"]["workspace_google_session"]["type"] = retired_type

    with pytest.raises(ValueError, match="use 'bundle'"):
        authority_provider_instances(registry)
