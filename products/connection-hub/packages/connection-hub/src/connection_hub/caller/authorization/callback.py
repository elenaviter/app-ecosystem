"""The loopback callback that receives a browser authorization response.

Adapted from KDCube's ``kdcube_cli.management.callback`` (MIT, same copyright
holder) so the caller layer needs no KDCube package (W322).
"""
...
from connection_hub.caller.authorization.validation import validate_web_url
    ...
    class CallbackHandler(BaseHTTPRequestHandler):
        ...
        def _respond(self, status: int, message: str) -> None:
            body = (
                ...
                "<title>Connection Hub</title></head><body><p>"
...
class LoopbackCallbackServer:
    def __init__(
        ...
        *,
        ...
        # The native-client DCR policy allows an ephemeral loopback port
        ...
        try:
            ...
            raise AuthorizationError(
                ...
                "Could not open a local authorization callback.",
            ) from exc
        self._server.daemon_threads = True
        self._thread = threading.Thread(
            target=self._server.serve_forever,
            name="connection-hub-authorization-callback",
            daemon=True,
        )
        self._closed = False
        self._thread.start()
