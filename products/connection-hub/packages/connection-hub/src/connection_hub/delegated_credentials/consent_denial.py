...
RequirementsLoader = Callable[..., Awaitable[Sequence[Mapping[str, Any]]]]
AccountsLoader = Callable[..., Awaitable[Sequence[Any]]]
ConsentPayloadBuilder = Callable[..., dict[str, Any]]
...
async def connect_first_denial(
    ...
    *,
    ...
    project: str,
    hub_bundle_id: str = "connection-hub@1-0",
    requirements_loader: RequirementsLoader | None = None,
    accounts_loader: AccountsLoader | None = None,
    consent_payload_builder: ConsentPayloadBuilder | None = None,
    ...
    return await connect_first_denial_for_identity(
        ...
        requirements_loader=requirements_loader,
        accounts_loader=accounts_loader,
        consent_payload_builder=consent_payload_builder,
    ...
    *,
    ...
    if explicit:
        ...
        if requirements_loader is None:
            LOGGER.warning(
                "[connect-first] -> None (provider requirements loader is not configured)"
            )
            return None
        try:
            discovered = await requirements_loader(
                namespace=namespace,
                tenant=tenant,
                project=project,
            )
            ...
            return None
        discovered = [item for item in (discovered or ()) if isinstance(item, Mapping)]
        LOGGER.info(
            "[connect-first] discovery namespace=%s requirements=%s providers=%s",
            namespace, len(discovered),
    ...
    for req in requirements:
        ...
        if accounts_loader is None:
            LOGGER.warning(
                "[connect-first] -> None (account loader is not configured, provider=%s)",
                provider_id,
            )
            return None
        try:
            accounts = await accounts_loader(
        ...
        if connected:
            ...
            continue
        if consent_payload_builder is None or connector_app_resolver is None:
            LOGGER.warning(
                "[connect-first] -> None (consent payload ports are not configured)"
            )
            return None

        # One rule: the guarded service's declaration, or provider-wide.
        connector_app_id = connector_app_resolver(provider_id)
        payload = consent_payload_builder(
            tenant=tenant,
            project=project,
