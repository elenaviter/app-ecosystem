...
"""Credential authority contracts and the local provider registry.

Redis-backed provider discovery is implemented in
``connection_hub.authority_discovery`` and re-exported lazily here to preserve
the package's original public import path.
"""
...
class CredentialEnvelope:
    ...
    existing token format such as ``kst1``. It is not authorization by itself;
    it tells the SDK which authority/authenticator can attempt verification.
    """
    ...
    def public_claim(self) -> dict[str, Any]:
        """Return the safe subset suitable for embedding in a signed token."""
...
class AuthorityProviderSpec:
    ...
    def from_dict(cls, value: Mapping[str, Any] | None) -> "AuthorityProviderSpec":
        data = dict(value or {})
        authority_id = _str(
            data.get("authority_id") or data.get("id") or data.get("authority")
        )
        return cls(
            authority_id=authority_id,
            provider_id=_str(
                data.get("provider_id") or data.get("provider") or authority_id
            ),
            bundle_id=_str(data.get("bundle_id")),
            label=_str(data.get("label")),
            credential_kinds=_list(
                data.get("credential_kinds") or data.get("kinds")
            ),
            audiences=_list(data.get("audiences") or data.get("audience")),
            authenticators=_list(
                data.get("authenticators") or data.get("authenticator_ids")
            ),
    ...
    def matches(self, credential: CredentialEnvelope | Mapping[str, Any]) -> bool:
        ...
        if (
            self.authority_id
            and env.issuer_authority_id
            and self.authority_id != env.issuer_authority_id
        ):
            return False
        if (
            self.authenticators
            and env.issuer_authenticator_id
            and env.issuer_authenticator_id not in self.authenticators
        ):
            return False
        if (
            self.credential_kinds
            and env.credential_kind
            and env.credential_kind not in self.credential_kinds
        ):
            return False
...
class AuthorityRegistry:
    ...
    def register(
        ...
        if provider_spec.authority_id in self._providers:
            raise ValueError(
                f"authority provider already registered: {provider_spec.authority_id}"
            )
    ...
    def resolve(
        self,
        credential: CredentialEnvelope | Mapping[str, Any],
    ) -> RegisteredAuthorityProvider | None:
        env = CredentialEnvelope.coerce(credential)
        if env.issuer_authority_id:
            ...
            return None
        matches = [
            entry for entry in self._providers.values() if entry.spec.matches(env)
        ]
        if len(matches) == 1:
            return matches[0]
        if len(matches) > 1:
            raise ValueError(
                "multiple authority providers match credential; "
                "include issuer_authority_id"
            )
        return None
    ...
    async def verify(
        ...
        *,
        ...
        return await entry.provider.verify_credential(
            env,
            token=token,
            context=context,
        )
...
def authority_provider_spec_from_declaration(
    ...
    *,
    ...
    data = _dict(declaration)
    authority_id = _str(
        getattr(declaration, "authority_id", None) or data.get("authority_id")
    )
    authenticator_id = _str(
        getattr(declaration, "authenticator_id", None)
        or data.get("authenticator_id")
    )
    credential_kinds = _list(
        getattr(declaration, "credential_kinds", None)
        or data.get("credential_kinds")
    )
    audiences = _list(
        getattr(declaration, "audiences", None) or data.get("audiences")
    )
    transports = _list(
        getattr(declaration, "transports", None) or data.get("transports")
    ) or ("local",)
    label = _str(getattr(declaration, "label", None) or data.get("label"))
    return AuthorityProviderSpec(
        authority_id=authority_id,
        provider_id=authority_id,
        bundle_id=_str(bundle_id or data.get("bundle_id")),
        label=label,
...
def __getattr__(name: str) -> Any:
    if name not in _DISCOVERY_EXPORTS:
        raise AttributeError(name)
    module = import_module("connection_hub.authority_discovery")
