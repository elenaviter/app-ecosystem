...
ContextUpdater = Callable[[Mapping[str, Any]], Any]


def set_service_connector_apps(
    ...
    *,
    ...
    context_updater({_BCC_KEY: cleaned})


def resolve_connector_app_id(
    provider_id: str,
    *,
    ...
    current = context
    if current is None:
        current = context_reader() if context_reader is not None else {}
    declaration = current.get(_BCC_KEY)
