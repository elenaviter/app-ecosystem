...
class AuthorityRegistryClient:
    ...
    def __init__(
        ...
        *,
        ...
        self._bundle_props_loader = bundle_props_loader
    ...
    async def _load_connection_hub_bundle_props(self) -> dict[str, Any]:
        ...
        if self.redis is not None and self.tenant and self.project:
            if self._bundle_props_loader is None:
                raise RuntimeError(
                    "AuthorityRegistryClient requires a bundle-props loader "
                    "when Redis-backed configuration is selected"
                )
            props = await self._bundle_props_loader(
                self.redis,
                tenant=self.tenant,
...
__all__ = ["AuthorityRegistryClient", "BundlePropsLoader"]
