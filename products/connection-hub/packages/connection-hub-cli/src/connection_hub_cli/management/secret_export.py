...
from connection_hub_cli.authorization.models import validate_web_url
...
def _fixed_error(
    ...
    return AuthorizationError(exact, message)
...
class SecretExportClient:
    ...
    async def start(self, request: SecretExportRequest) -> SecretExportStart:
        ...
        if set(payload) != {
            ...
            "required_assurance",
            "expires_at",
        } or payload.get("schema") != SECRET_EXPORT_START_SCHEMA or payload.get(
            "ok"
        ) is not True:
    ...
    async def exchange(
        ...
        *,
        ...
        if set(payload) != {
            ...
            "approval",
            "values",
        } or payload.get("schema") != SECRET_EXPORT_RESULT_SCHEMA or payload.get(
            "ok"
        ) is not True:
            raise _fixed_error("secret_export_response_invalid")
        if (
        ...
        except (AuthorizationError, UnicodeError, ValueError):
            raise _fixed_error("secret_export_response_invalid") from None
        ...
        try:
            ...
            try:
                ...
                raise _fixed_error(
                    ...
                    "Connection Hub CLI could not open the secret export approval page.",
                ) from None
            if not opened:
                raise _fixed_error(
                    "secret_export_browser_open_failed",
