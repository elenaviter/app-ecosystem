...
ADMISSION_SCHEMA = "connection_hub.delegated_admission.v1"
# Kept stable so signatures issued by the alpha predecessor distribution
# remain verifiable after the package-name migration.
ADMISSION_SIGNATURE_VERSION = "prokura-admission-v1"

SERVICE_ID_HEADER = "x-connection-hub-service-id"
SERVICE_TIMESTAMP_HEADER = "x-connection-hub-timestamp"
SERVICE_NONCE_HEADER = "x-connection-hub-nonce"
SERVICE_SIGNATURE_HEADER = "x-connection-hub-signature"

_LEGACY_SERVICE_ID_HEADER = "x-prokura-service-id"
_LEGACY_SERVICE_TIMESTAMP_HEADER = "x-prokura-timestamp"
_LEGACY_SERVICE_NONCE_HEADER = "x-prokura-nonce"
_LEGACY_SERVICE_SIGNATURE_HEADER = "x-prokura-signature"
...
class ServiceProof:
    ...
    def from_headers(cls, headers: Mapping[str, Any] | None) -> "ServiceProof":
        ...
        return cls(
            service_id=_text(
                normalized.get(SERVICE_ID_HEADER)
                or normalized.get(_LEGACY_SERVICE_ID_HEADER)
            ),
            timestamp=_text(
                normalized.get(SERVICE_TIMESTAMP_HEADER)
                or normalized.get(_LEGACY_SERVICE_TIMESTAMP_HEADER)
            ),
            nonce=_text(
                normalized.get(SERVICE_NONCE_HEADER)
                or normalized.get(_LEGACY_SERVICE_NONCE_HEADER)
            ),
            signature=_text(
                normalized.get(SERVICE_SIGNATURE_HEADER)
                or normalized.get(_LEGACY_SERVICE_SIGNATURE_HEADER)
            ),
        )
...
def pairwise_service_subject(
    *,
    ...
    # The domain separator is persisted identity behavior, not product copy.
    ...
    *,
    ...
    return {
        ...
        "error": {
            ...
            "where": "connection_hub.delegated_admission",
