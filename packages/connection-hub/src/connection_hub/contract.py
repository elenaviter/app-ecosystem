...
class ConnectionOperationSpec(Mapping[str, Any]):
    """Transport-neutral operation declaration.

    The mapping interface lets host registries consume the declaration through
    their ordinary mapping-coercion path. Attribute access remains available
    to package callers that inspect the operation inventory directly.
    """
    ...
    def __iter__(self) -> Iterator[str]:
        return iter(("operation", "transports"))

    def __len__(self) -> int:
        return 2

    def to_dict(self) -> dict[str, Any]:
        return {"operation": self.operation, "transports": list(self.transports)}
