...
class UserConfigurationStore(Protocol):
    """Host storage for one user's properties and server-side secrets."""
...
class DelegatedToKdcubeStore:
    ...
    async def _prop(self, key: str, default: Any = None) -> Any:
        return await self._backend.get_user_prop(
            key,
    ...
    async def _set_prop(self, key: str, value: Any) -> None:
        await self._backend.set_user_prop(
            key,
            value,
            user_id=self.user_id,
            bundle_id=self.bundle_id,
    ...
    async def _delete_prop(self, key: str) -> None:
        await self._backend.delete_user_prop(
            key,
            user_id=self.user_id,
            bundle_id=self.bundle_id,
        )
    ...
    async def set_credential(self, credential_id: str, credential: dict[str, Any]) -> None:
        ...
        await self._backend.set_user_secret(
    ...
    async def delete_credential(self, credential_id: str) -> None:
        if not credential_id:
            return
        await self._backend.delete_user_secret(
            self.credential_secret_key(credential_id),
            user_id=self.user_id,
            bundle_id=self.bundle_id,
        )
