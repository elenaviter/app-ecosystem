...
from connection_hub.delegated_to_kdcube.broker import (
    DelegatedToKdcubeBroker,
    broker_for_user,
)
from connection_hub.delegated_to_kdcube.config import (
...
SecretLoader = Callable[..., Awaitable[Any]]
StoreFactory = Callable[..., DelegatedToKdcubeStore]
        ...
        *,
        ...
        if redis is not None and resolved_tenant and resolved_project and bundle_id:
            if bundle_props_loader is None:
                raise RuntimeError(
                    "from_connection_hub requires a bundle-props loader for Redis-backed configuration"
                )
            props = dict(
                await bundle_props_loader(
                    redis,
        ...
        async def _client_secret_resolver(*, provider_id: str, connector_app_id: str, connector_app: Any) -> str:
            ...
            return _clean(
                await secret_loader(secret_ref, bundle_id=bundle_id)
