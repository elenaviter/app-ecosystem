...
from prokura.delegated_to_kdcube.adapters import resolve_adapter
from prokura.delegated_to_kdcube.broker import DelegatedToKdcubeBroker
...
from prokura.delegated_to_kdcube.oauth import (
    OAuthStateStore,
    consume_oauth_state,
    create_oauth_state,
    state_digest,
)
from prokura.delegated_to_kdcube.store import (
...
def operations_for_user(
    *,
    ...
    store_factory: Callable[..., DelegatedToKdcubeStore] | None = None,
    consent_granted_notifier: Callable[..., Any] | None = None,
    ...
    if resolved_store is None:
        if store_factory is None:
            raise RuntimeError(
                "operations_for_user requires a connected-account store factory"
            )
        resolved_store = store_factory(user_id=user_id, bundle_id=bundle_id)
