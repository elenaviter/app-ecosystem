...
from prokura.connection_edges import DEFAULT_CONNECTION_HUB_BUNDLE_ID
from prokura.delegated_to_kdcube.client import DelegatedToKdcubeClient
...
def connected_account_consent_payload(
    *,
    ...
    if reason == REASON_AGENT_GRANT_REQUIRED and as_str(agent_client_id) and as_str(agent_resource):
        ...
        from prokura.delegated_credentials.consent_denial import (
...
async def preflight_tool_claim_policies(
    *,
    ...
    if client_factory is None:
        raise RuntimeError(
            "preflight_tool_claim_policies requires a connected-account client factory"
        )
