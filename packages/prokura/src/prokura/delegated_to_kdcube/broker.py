...
def broker_for_user(
    *,
    ...
    from prokura.delegated_to_kdcube.models import CONNECTION_HUB_BUNDLE_ID

    resolved_store = store
    if resolved_store is None:
        if store_factory is None:
            raise RuntimeError("broker_for_user requires a connected-account store factory")
        resolved_store = store_factory(
            user_id=user_id,
...
__all__ = ["DelegatedToKdcubeBroker", "StoreFactory", "broker_for_user"]
