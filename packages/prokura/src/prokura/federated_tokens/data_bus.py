...
SecretLoader = Callable[[str], Awaitable[str | bytes | None]]
TokenKeyBuilder = Callable[..., str]
...
async def _resolve_secret(
    ...
    *,
    ...
    try:
        resolved = await secret_loader(FEDERATED_TOKEN_SECRET_KEY)
    except Exception as exc:
        raise FederatedTokenInvalid(
            f"federated token secret is not configured at {FEDERATED_TOKEN_SECRET_KEY}"
        ) from exc
...
def _token_key(
    *,
    ...
    project: str,
    jti: str,
    token_key_builder: TokenKeyBuilder | None = None,
) -> str:
    base = f"{FEDERATED_TOKEN_REDIS_BASE}:{jti}"
    if token_key_builder is not None:
        return token_key_builder(base, tenant=tenant, project=project)
    return f"{tenant}:{project}:{base}"
...
async def issue_federated_data_bus_token(
    *,
    ...
    resolved_secret = await _resolve_secret(secret, secret_loader=secret_loader)
    ...
    await redis.setex(
        _token_key(
            ...
            project=project_value,
            jti=claims["jti"],
            token_key_builder=token_key_builder,
        ),
        ttl,
