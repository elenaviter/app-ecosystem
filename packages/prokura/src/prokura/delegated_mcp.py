...
def self_hosted_url(
    ...
    url: str,
    *,
    runtime_local_base: str = "http://127.0.0.1:8020",
) -> str:
    ...
    if not raw:
        return url
    base = (
        str(conn.get("internal_base_url") or "").strip().rstrip("/")
        or str(runtime_local_base or "").strip().rstrip("/")
    )
    try:
    ...
    *,
    ...
    for conn in connections or []:
        ...
        if not server_id or not url:
            continue
        dial_url = self_hosted_url(
            conn,
            str(url),
            runtime_local_base=runtime_local_base,
        )
        if dial_url != url:
            logger.info(
