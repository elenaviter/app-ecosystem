...
PLATFORM_PRIVILEGED_ROLE_GRANTS: tuple[str, ...] = tuple(
    sorted(("kdcube:role:admin", "kdcube:role:super-admin"))
)
...
def authority_has_platform_privilege(
    roles: Iterable[Any] | None,
    *,
    privileged_roles: Iterable[Any] = PLATFORM_PRIVILEGED_ROLE_GRANTS,
) -> bool:
    return bool(
        set(normalize_authority_values(roles))
        & set(normalize_authority_values(privileged_roles))
    )
