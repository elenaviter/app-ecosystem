...
CONNECTION_HUB_SERVICE_ID           = reference-customers-api
CONNECTION_HUB_SERVICE_SECRET       = registered workload secret
CONNECTION_HUB_RESOURCE             = https://reference.example.test/customers
```
...
This is a host route, not Connection Hub's permanent product URL. A shorter stable
...
Connection Hub answers whether the delegated caller currently holds the configured
