---
id: connection-hub-package-extraction-architecture
title: Connection Hub Package Boundary And Extraction
summary: Defines the authority domain Connection Hub owns, the host-runtime ports it consumes, and the completed source cutover from KDCube without changing delegated-access behavior.
tags:
  - connection-hub
  - architecture
  - delegated-access
...
# Connection Hub package boundary and extraction

Connection Hub owns the delegated-access authority. A caller presents its identity
and the operation it wants; Connection Hub resolves that request against the current
...
`connection-hub`. KDCube consumers import portable contracts directly from that
package and use focused host adapters under
`kdcube_ai_app.apps.chat.sdk.integrations.connection_hub`. The historical KDCube
...
embedded in another service or exposed by Connection Hub's planned standalone
service.
...
1. Move the closed, host-neutral model and state-machine core into `connection-hub`.
...
3. Move the Connection Hub product application into this repository with
   Connection Hub as its portable delegated-authority engine.
...
5. Retire the built-in Connection Hub app and historical KDCube connections
   package, then migrate consumers to direct `connection-hub` imports or the explicit
