---
id: docs/connection-hub/package/server-side-login
title: "Server-side login: one platform session for every surface"
...
`connection_hub.server_side_login` is a host-neutral implementation of the
backend-for-frontend login pattern. The browser holds one
HttpOnly, Secure, SameSite=Lax cookie with a signed session token. The server
holds the identity result, renews nothing in the browser, and every surface
...
| `IdTokenVerifier` | Signature, issuer, audience and expiry of an authenticator's ID token. `oidc_jwt.PyJwtVerifier` when PyJWT is installed (`pip install connection-hub[oidc]`). |
...
logout(token)                          -> LogoutCompleted: the cleared cookie, the authenticator logout URL
```
