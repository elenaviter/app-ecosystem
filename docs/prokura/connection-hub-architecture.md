---
id: prokura/connection-hub-architecture
title: "Connection Hub Architecture And Semantic Requirements"
summary: "Canonical architecture for the Prokura-backed Connection Hub app: identity and delegation semantics, public and managed surfaces, storage authorities, host boundaries, and direct protected-service admission."
status: active
tags: ["prokura", "connection-hub", "architecture", "identity", "delegated-access", "storage", "admission"]
...
Connection Hub is the Prokura-backed application through which users and
...
`connection-hub@1-0`. Prokura owns portable authority models and decisions;
...
```json
{
  "ok": true,
  "allowed": true,
  "schema": "prokura.delegated_admission.v1",
...
Denials use `prokura.delegated_admission.v1`, a stable error code, a
...
The `prokura` package supplies the canonical request and signing functions:
...
standalone service must supply equivalent ports; it must not fork Prokura's
